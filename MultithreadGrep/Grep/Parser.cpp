...
namespace multigrep {
	...
	void searchDirectory(std::string path) {
		if (sharedvars::params_ptr->verbose) {
			{
				std::lock_guard<std::mutex> lk(sharedvars::printMtx);
				std::cout << "Scanning: " << boost::filesystem::system_complete(path) << std::endl;
			}
		}
		try {
							...
							if (sharedvars::params_ptr->verbose) {
								{
									...
									std::lock_guard<std::mutex> lk(sharedvars::printMtx);
									std::cout << "Grepping: \"" << boost::filesystem::system_complete(iter->path()).string() << "\"" << std::endl;
